...
# This code assumes that there is a function named nextLifeGeneration in the
# file hw8pr1.py.

# Revision notes:
# 1. Comment out the line 'screen.onkey(eraseBoard, "space")' in the
#    'newWindow()' function
#    to avoid erasing screen by
#    accidentally hitting the space key
#    by X. Meng 2016-11-2
#
# 2. Change the use of 'input()' to 'sys.stdin.read()'
#    in the 'readBoard()' function so a matrix with
#    multiple lines can be properly read.
#    by X. Meng 2016-10-31

import sys     # added by Xiannong Meng 2016-10-31 for sys.stdin.read()
import time
from turtle import *
from hw8pr1 import *
...
def displayNextLifeGeneration():
    ...
    currentBoard = addBorder(nextLifeGeneration(removeBorder(currentBoard)))
...
def runGameOfLife():
    ...
    screen.listen()
    if isGameRunning():
         nextGeneration = nextLifeGeneration(removeBorder(currentBoard))
         followingGeneration = nextLifeGeneration(nextGeneration)
...
def newWindow():
    ...
    screen.onkey(bye, "q")
    # screen.onkey(eraseBoard, "space")   # Comment out by X. Meng 2016-11-2
                                          # to avoid erasing screen by
                                          # accidentally hitting the space key
...
def readBoard():
    ...
    """
    #s = input()     # Changed to sys.stdin.read() X. Meng 2016-10-31
    s = sys.stdin.read()
    s = s.strip()
    s = s.replace("\t", "")
    s = s.replace(" ", "")
...
def readBoardWithPadding(rowPad, colPad):
