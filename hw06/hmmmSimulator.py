#!/usr/remote/anaconda-3.5/bin/python
# hmmmSimulator.py
#
...
#  3. In the function 'valid_integer()', the original intension was that
#     the function will return True only if the parameter is of the integer
#     type AND it is within the specified range. The function will return
#     False for any other parameter, e.g., a string. It is feasible in Python 2
#     to implement the function as
#     def valid_integer(x):
#       return -32768 <= x <= 32767
#     where if parameter 'x' is a string, the function will return False.
#     In Python 3, the comparison between an integer and a string is illegal.
#     Thus, the function has to check the type of the parameter first.
#     def valid_integer(x):
#        if not isinstance(x, int):
#           return False
#        return -32768 <= x <= 32767
#  4. For the moment, Python 3 in our system is under /usr/remtoe/anaconda-3.5
...
def clear_num(x):
    """Remove leading and trailing non-digits from the string 'x'"""
    print('clear_num : ', x)
    y = x.lstrip(' \n\t')
    x = y.rstrip(' \n\t').replace(' ', '')
    return x
...
def instToNum(x):
    """Convert an instruction to a number"""
    x = clear_num(x)
    #print('ir value ', BinaryToNum(x))
...
def execute(ir) :
    ...
    #print('execute: ir ', ir)
    #print(valid_integer(instToNum(ir)))
    if ir == "" or valid_integer(ir):
        simulationError("Bad instruction at memory location " + str(lpc))
    ...
    if opcode == "halt":  
        ...
        while val == "" \
          or  (not (val.isdigit() \
            or (val[0] == '-' and val[1:].isdigit()))) \
          or not valid_integer(int(val)):
            print( "\n\nIllegal input: number must be in [-32768,32767]" )
            val = input("Enter number (q to quit): ")
            if val == "q" :
                sys.exit()
        register[args[0]] = int(val) 
